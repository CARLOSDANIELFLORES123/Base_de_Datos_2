...
CREATE OR REPLACE VIEW  autores_peru_history AS
    select cat.nombre AS categoria,
        au.nombre AS autor,
        au.nacionalidad AS nacionalidad
    FROM libro AS lib
...
select  getNombreCompleto('Carlos Daniel Flores Paucara');

CREATE FUNCTION suma3Numeros(a int, b int, c int)
RETURNS INT
BEGIN
    declare R INT;
    set R=a+b+c;
...
select suma3Numeros(4,3,8);


CREATE OR REPLACE FUNCTION calculadora(num1 int, num2 int, operacion varchar(50))
RETURNS INT
BEGIN
...
WHEN 'Multi'THEN SET resultado = num1 * num2;
...
select valida_historia_peru(cat.nombre,nac.nacionalidad );